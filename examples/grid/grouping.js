...
Ext.grid.dummyData = [
   ...
   // adding more data to test print on multiple pages
...
// make reader globally available (for onReady AND onPrinting)
...
// make store globally available (for onReady AND onPrinting)
...
// make columns globally available (for onReady AND onPrinting)
columns = [
...
Ext.onPrinting(function(){
  ...
  Ext.addStyleSheet("grouping-printer-friendly.css");
  ...
  new Ext.Panel({
    renderTo: "body", // assuming the body tag has the id "body"
    layout: "table",
    layoutConfig: {
        columns: 1
    },
    ...
    items: [
        new Ext.BoxComponent({ 
            el: 'description' // assuming the description is located within a div with id "description"
        }),
        
        // Special PrinterGridPanel which is optimized for performance for printing large data sets,
        // and which shows all rows (since you can't scoll or 'paginate' on paper)
        new Ext.ux.grid.PrinterGridPanel({
            store: store,
            columns: columns,
            view: new Ext.ux.grid.PrinterGroupingView({
                groupTextTpl: '{text} ({[values.rs.length]} {[values.rs.length > 1 ? "Items" : "Item"]})'
            }),
            stateId: 'grouping-example',
